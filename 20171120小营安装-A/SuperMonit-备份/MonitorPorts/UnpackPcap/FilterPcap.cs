        ...
        public List<string> getReqrFileList(DateTime _startTime, DateTime _endTime, string path)
        {
            ...
            foreach (DirectoryInfo YearFolder in fileFolder.GetDirectories())          //在保存pcap文件的目录下查找符合条件的文件
            {
                ...
                else if (_startTime.Year == Convert.ToInt16(YearFolder.Name) && _endTime.Year > Convert.ToInt16(YearFolder.Name))          // 年份文件夹的年份等于筛选的起始年份，小于终止年份
                {
                    foreach (DirectoryInfo MonthFolder in YearFolder.GetDirectories())
                    {
                        if (Convert.ToInt16(MonthFolder.Name) == _startTime.Month)                    //月份文件夹的月份等于筛选的起始月份
                        {
                            foreach (DirectoryInfo DayFolder in MonthFolder.GetDirectories())
                            {
                                if (Convert.ToInt16(DayFolder.Name) >= _startTime.Day)                //日期文件夹日期大于筛选的起始日期，满足条件
                                {
                        ...
                        if (Convert.ToInt16(MonthFolder.Name) > _startTime.Month)                   // 月份文件夹的月份大于筛选的起始月份，文件夹下所有文件满筛选条件
                        {
                            foreach (DirectoryInfo DayFolder in MonthFolder.GetDirectories())
                            {
                                foreach (FileInfo file in DayFolder.GetFiles("*.pcap"))
                ...
                else if (_startTime.Year < Convert.ToInt16(YearFolder.Name) && _endTime.Year == Convert.ToInt16(YearFolder.Name))           ///年份文件夹的年份大于筛选的起始年份，等于于终止年份
                {
                    foreach (DirectoryInfo MonthFolder in YearFolder.GetDirectories())
                    {
                        if (Convert.ToInt16(MonthFolder.Name) == _endTime.Month)                                //月份文件夹的月份等于筛选终止月份
                        {
                            foreach (DirectoryInfo DayFolder in MonthFolder.GetDirectories())
                            {
                                if (Convert.ToInt16(DayFolder.Name) <= _endTime.Day)                               //日期文件夹日期小于筛选的终止日期，满足条件
                                {
                                    foreach (FileInfo file in DayFolder.GetFiles("*.pcap"))
                                    {
                                        string s = file.ToString();
                        ...
                        if (Convert.ToInt16(MonthFolder.Name) < _endTime.Month)                         //月份文件夹的月份小于筛选终止月份，文件夹下所有文件满筛选条件
                        {
                            foreach (DirectoryInfo DayFolder in MonthFolder.GetDirectories())
                            {
                                foreach (FileInfo file in DayFolder.GetFiles("*.pcap"))
                ...
                else if (_startTime.Year == Convert.ToInt16(YearFolder.Name) && _endTime.Year == Convert.ToInt16(YearFolder.Name))       ///年份文件夹的年份等于筛选的起始年份，也等于终止年份
                {
                    foreach (DirectoryInfo MonthFolder in YearFolder.GetDirectories())
                    {
                        if (Convert.ToInt16(MonthFolder.Name) < _endTime.Month && Convert.ToInt16(MonthFolder.Name) > _startTime.Month)           //月份文件夹的月份大于筛选的起始月份，小于终止月份，该文件夹下所有文件满足
                        {
                            foreach (DirectoryInfo DayFolder in MonthFolder.GetDirectories())
                            {
                                foreach (FileInfo file in DayFolder.GetFiles("*.pcap"))
                        ...
                        if (Convert.ToInt16(MonthFolder.Name) == _endTime.Month && Convert.ToInt16(MonthFolder.Name) > _startTime.Month)              //月份文件夹的月份大于筛选的起始月份，等于终止月份
                        {
                            foreach (DirectoryInfo DayFolder in MonthFolder.GetDirectories())
                            {
                                if (Convert.ToInt16(DayFolder.Name) <= _endTime.Day)                                                 //   日期文件夹的日期小于筛选的终止日期，满足条件
                                {
                                    foreach (FileInfo file in DayFolder.GetFiles("*.pcap"))
                                    {
                                        string s = file.ToString();
                        ...
                        if (Convert.ToInt16(MonthFolder.Name) < _endTime.Month && Convert.ToInt16(MonthFolder.Name) == _startTime.Month)             //月份文件夹的月份等于筛选的起始月份，小于终止月份
                        {
                            foreach (DirectoryInfo DayFolder in MonthFolder.GetDirectories())
                            {
                                if (Convert.ToInt16(DayFolder.Name) >= _startTime.Day)                                       //日期文件夹的日期大于筛选的起始日期，满足条件
                                {
                                    foreach (FileInfo file in DayFolder.GetFiles("*.pcap"))
                                    {
                                        string s = file.ToString();
                        ...
                        if (Convert.ToInt16(MonthFolder.Name) == _endTime.Month && Convert.ToInt16(MonthFolder.Name) == _startTime.Month)      //筛选条件为同一月份
                        {
                            foreach (DirectoryInfo DayFolder in MonthFolder.GetDirectories())
                            {
                                if (Convert.ToInt16(DayFolder.Name) <= _endTime.Day && Convert.ToInt16(DayFolder.Name) >= _startTime.Day)      //日期文件夹的日期应满足大于筛选起始日期小于筛选终止日期
                                {
                                    foreach (FileInfo file in DayFolder.GetFiles("*.pcap"))
                                    {
                                        string s = file.ToString();
